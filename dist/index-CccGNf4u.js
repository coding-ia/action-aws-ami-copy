...
import 'path';
import {
  C as CredentialsProviderError,
  s as setCredentialFeature,
  j as chain,
  i as getProfileName,
} from './main-y3006xry.js';
import 'crypto';
import { p as parseKnownFiles } from './parseKnownFiles-y4yefOge.js';
import 'fs';
import 'http';
import 'https';
import 'net';
...
const resolveCredentialSource = (credentialSource, profileName, logger) => {
  const sourceProvidersMap = {
    EcsContainer: async (options) => {
      const { fromHttp } = await import('./index-qLzbyGor.js');
      const { fromContainerMetadata } = await import('./index-CXOC_q6h.js');
      logger?.debug(
        '@aws-sdk/credential-provider-ini - credential_source is EcsContainer',
      );
      return async () =>
        chain(fromHttp(options ?? {}), fromContainerMetadata(options))().then(
          setNamedProvider,
        );
    },
    Ec2InstanceMetadata: async (options) => {
      ...
      const { fromInstanceMetadata } = await import('./index-CXOC_q6h.js');
    ...
    Environment: async (options) => {
      ...
      const { fromEnv } = await import('./index-BNuGZ9W6.js');
...
const resolveAssumeRoleCredentials = async (
  ...
  if (!options.roleAssumer) {
    const { getDefaultRoleAssumer } = await import('./index-DOmQUwBX.js');
  ...
  if (isCredentialSourceWithoutRoleArn(profileData)) {
    ...
    const params = {
      ...
      RoleSessionName:
        profileData.role_session_name || `aws-sdk-js-${Date.now()}`,
...
const resolveProcessCredentials = async (options, profile) =>
  import('./index-DnIRBEzs.js').then(({ fromProcess }) =>
    fromProcess({
      ...options,
      profile,
    })().then((creds) =>
      setCredentialFeature(creds, 'CREDENTIALS_PROFILE_PROCESS', 'v'),
    ),
  );

const resolveSsoCredentials = async (profile, profileData, options = {}) => {
  const { fromSSO } = await import('./index-W-zR-Txj.js');
  return fromSSO({
    profile,
    logger: options.logger,
...
const resolveWebIdentityCredentials = async (profile, options) =>
  import('./index-B5cxr9xB.js').then(({ fromTokenFile }) =>
    fromTokenFile({
      ...
      setCredentialFeature(creds, 'CREDENTIALS_PROFILE_STS_WEB_ID_TOKEN', 'q'),
    ),
  );
...
const resolveProfileData = async (
  ...
  if (
    ...
    isAssumeRoleProfile(data, { profile: profileName, logger: options.logger })
  ) {
...
export { fromIni };
//# sourceMappingURL=index-CccGNf4u.js.map
