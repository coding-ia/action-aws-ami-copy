import {
  ...
  m as toHex,
  n as toUtf8,
  o as fromArrayBuffer,
  p as streamCollector$1,
  C as CredentialsProviderError,
  H as HttpRequest,
  q as parseRfc3339DateTime,
...
const isReadableStream = (stream) =>
  ...
  (stream?.constructor?.name === ReadableStream.name ||
    stream instanceof ReadableStream);

const streamCollector = async (stream) => {
  if (
    (typeof Blob === 'function' && stream instanceof Blob) ||
    stream.constructor?.name === 'Blob'
  ) {
...
const ERR_MSG_STREAM_HAS_BEEN_TRANSFORMED$1 =
...
const AWS_CONTAINER_AUTHORIZATION_TOKEN_FILE =
    ...
    async () => {
      ...
      if (token) {
        ...
        request.headers.Authorization = (
          await fs.readFile(tokenFile)
        ).toString();
      }
      try {
        const result = await requestHandler.handle(request);
        return getCredentials(result.response).then((creds) =>
          setCredentialFeature(creds, 'CREDENTIALS_HTTP', 'z'),
        );
      } catch (e) {
...
export { fromHttp };
//# sourceMappingURL=index-qLzbyGor.js.map
