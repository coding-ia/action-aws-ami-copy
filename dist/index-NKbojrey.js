...
import 'crypto';
import { p as parseKnownFiles } from './parseKnownFiles-DIUjzGOa.js';
import 'fs';
import 'http';
import 'https';
import 'net';
...
const resolveCredentialSource = (credentialSource, profileName, logger) => {
  const sourceProvidersMap = {
    EcsContainer: async (options) => {
      const { fromHttp } = await import('./index-CVUXjNvA.js');
      const { fromContainerMetadata } = await import('./index-CZqj87S3.js');
      logger?.debug(
    ...
    Ec2InstanceMetadata: async (options) => {
      ...
      const { fromInstanceMetadata } = await import('./index-CZqj87S3.js');
    ...
    Environment: async (options) => {
      ...
      const { fromEnv } = await import('./index-CjKghlZ8.js');
...
const resolveAssumeRoleCredentials = async (
  ...
  if (!options.roleAssumer) {
    const { getDefaultRoleAssumer } = await import('./index-D97-sN_T.js');
...
const resolveProcessCredentials = async (options, profile) =>
  import('./index-CWYNur3s.js').then(({ fromProcess }) =>
    fromProcess({
      ...options,
      profile,
    })().then((creds) =>
...
const resolveSsoCredentials = async (profile, profileData, options = {}) => {
  const { fromSSO } = await import('./index-nnQFHayi.js');
  return fromSSO({
    profile,
    logger: options.logger,
...
const resolveWebIdentityCredentials = async (profile, options) =>
  import('./index-BxF6ddau.js').then(({ fromTokenFile }) =>
    fromTokenFile({
...
export { fromIni };
//# sourceMappingURL=index-NKbojrey.js.map
